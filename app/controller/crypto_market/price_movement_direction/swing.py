...
from app.controller.crypto_market.price_movement_direction.indicators import calculate_ema, calculate_rsi, \
    calculate_macd
...
async def swing_analysis(symbol: str, session: AsyncSession):
    votes = {
        "LONG": 0,
        "SHORT": 0,
        "HOLD": 0
    ...
    # Встановлюємо часові рамки (останні 30 днів)
    ...
    # RSI
    rsi = calculate_rsi(close, 21).iloc[-1]
    if rsi < 30:
        votes["LONG"] += 1
